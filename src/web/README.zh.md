...
- 实现 [`WebSearchBackend`](backend.rs) / [`WebFetchBackend`](backend.rs)，返回 [`WebSearchResult`](types.rs) / [`WebFetchResult`](types.rs)，错误统一为 [`ToolError`](../tool.rs)（多为 `NETWORK_ERROR`）。
